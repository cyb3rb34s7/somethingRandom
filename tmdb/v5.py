...
Dates Updated: {stats.dates_updated}
...
"""
        ...
        with open(f"{self.log_dir}/statistics.log", 'w', encoding='utf-8') as f:
    ...
    def write_failed_items(self, failed_items: List[Dict]):
        ...
        with open(f"{self.log_dir}/failed_items.log", 'w', encoding='utf-8') as f:
            f.write(content)
...
class DatabaseManager:
    ...
    def update_date(self, primary_topic_id: int, release_date: str):
        ...
        try:
            ...
            self.performance_logger.error(f"Error updating date for {primary_topic_id}: {str(e)}")
            raise
    ...
    def process_item(self, item: MovieShow) -> bool:
        ...
        try:
            ...
            if item.type.lower() == 'movie':
                ...
                release_date = data.get('first_air_date', '').strip()
            
            if release_date:
                self.db_manager.update_date(item.primary_topic_id, release_date)
                self.stats.dates_updated += 1
