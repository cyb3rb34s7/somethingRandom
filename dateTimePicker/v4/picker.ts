...
export class DatetimePickerComponent implements OnInit, ControlValueAccessor {
  ...
  scrollToSelectedTime() {
    if (this.hoursContainer && this.minutesContainer && this.secondsContainer) {
      ...
      const secondIndex = this.seconds.findIndex(s => s === this.selectedSecond);
      const secondElement = this.secondsContainer.nativeElement.children[secondIndex];
      
      // Scroll to center the selected time precisely
      // (Add half of the difference between container height and item height)
      if (hourElement) {
        const containerHeight = this.hoursContainer.nativeElement.clientHeight;
        const itemHeight = hourElement.clientHeight;
        this.hoursContainer.nativeElement.scrollTop = hourElement.offsetTop - (containerHeight / 2) + (itemHeight / 2);
      }
      
      if (minuteElement) {
        const containerHeight = this.minutesContainer.nativeElement.clientHeight;
        const itemHeight = minuteElement.clientHeight;
        this.minutesContainer.nativeElement.scrollTop = minuteElement.offsetTop - (containerHeight / 2) + (itemHeight / 2);
      }
      
      if (secondElement) {
        const containerHeight = this.secondsContainer.nativeElement.clientHeight;
        const itemHeight = secondElement.clientHeight;
        this.secondsContainer.nativeElement.scrollTop = secondElement.offsetTop - (containerHeight / 2) + (itemHeight / 2);
      }
    }
  }
