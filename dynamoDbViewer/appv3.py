...
# Helper function to flatten complex objects for DataFrame display
def flatten_item(item, max_str_length=50):
    """Flatten DynamoDB item for DataFrame display"""
    flattened = {}
    for key, value in item.items():
        if isinstance(value, (dict, list)):
            # Convert complex types to JSON string (truncated for display)
            json_str = json.dumps(value, default=decimal_default)
            if len(json_str) > max_str_length:
                flattened[key] = json_str[:max_str_length] + "..."
            else:
                flattened[key] = json_str
        elif isinstance(value, Decimal):
            flattened[key] = float(value) if value % 1 else int(value)
        else:
            flattened[key] = value
    return flattened

# Helper function to convert items list to DataFrame safely
def items_to_dataframe(items):
    """Safely convert DynamoDB items to pandas DataFrame"""
    if not items:
        return pd.DataFrame()
    
    # Ensure items is a list
    if not isinstance(items, list):
        st.error(f"Expected list of items, got {type(items)}")
        return pd.DataFrame()
    ...
    try:
        ...
        try:
            # Fallback: create a simple DataFrame with JSON strings
            simple_items = [{"Item": json.dumps(item, default=decimal_default)} for item in items]
            return pd.DataFrame(simple_items)
        ...
        with tab2:
            ...
            if 'items' in st.session_state and st.session_state.items is not None and len(st.session_state.items) > 0:
                ...
                if df.empty:
                    ...
                    if len(items) > 0:
                        # Create a simple display for item selection
                        item_options = []
                        for idx, item in enumerate(items):
                            # Get first few key-value pairs for display
                            preview = ", ".join([f"{k}: {str(v)[:30]}" for k, v in list(item.items())[:3]])
