...
def scrape_imdb_chart(url, limit=100):
    """
    Scrape IMDb chart page and extract title names and IMDb IDs from JSON-LD data
                ...
                if data.get('@type') == 'ItemList' and 'itemListElement' in data:
                    ...
                    for item in item_list[:limit]:
                        if item.get('@type') == 'ListItem' and 'item' in item:
                            ...
                            # Extract name
                            title_name = movie_data.get('name')
                            
                            # Extract IMDb ID from URL (format: /title/tt1234567/)
                            url_path = movie_data.get('url', '')
                            imdb_id = None
                            
                            if '/title/' in url_path:
                                parts = url_path.split('/title/')
                                if len(parts) > 1:
                    ...
                    # Found the right data, break out
                    if entries:
                        break
